...
async function loadComments(force = false) {
  if (!state.video) return
  if (drawer.loading && !force) return
  drawer.loading = true
  drawer.error = ''
  try {
...
async function ensureCommentsLoaded() {
  if (!drawer.open || drawer.loading || drawer.comments.length > 0 || drawer.error) return
  await loadComments()
}
...
async function handleCommentsClick(event?: Event) {
  ...
  drawer.content = ''
  drawer.comments = []
  void commentApi.listAll(state.video.id)
    .then((comments) => {
...
<template>
  <AppShell full>
    <div class="page">
      ...
      <div class="wrap">
        ...
        <div v-else-if="state.video" class="stage" @click="togglePlayPause">
          ...
          <div class="actions">
            ...
            <button class="act" type="button" @click="handleCommentsClick">
      ...
      <div v-if="drawer.open" class="drawer-backdrop" @vue:mounted="ensureCommentsLoaded" @click.self="closeDrawer">
        <div class="drawer">
          ...
          <div class="drawer-foot">
            ...
            <div class="row" style="justify-content: space-between; margin-top: 8px">
              <button class="chip" type="button" :disabled="drawer.loading" @click="refreshComments">刷新</button>
